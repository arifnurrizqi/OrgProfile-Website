...
<body>
  <main class="max-w-7xl max-w mx-auto flex lg:py-5 pb-32 px-4 pt-4 lg:gap-8">
    ...
    <div class="flex flex-col w-full right-0">
      ...
      <h1 class="text-xl font-bold lg:mb-2.5">Edit Fokus Isu Startegis</h1>

      <form action="<?= base_url() ?>admin/manage/submit_edit_fokusIsu" method="POST" enctype="multipart/form-data">
        <div>
          ...
          <input type="hidden" name="id" value="<?= $fokusIsu->id; ?>">
          <input type="text" name="fokus" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan lingkup fokus" value="<?= $fokusIsu->lingkup ?>" required />
        </div>
        <div>
          ...
          <textarea type="text" name="poin" rows="4" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan Poin Fokus berdasarkan Lingkup diatas" value="" required><?= $fokusIsu->poin_fokus ?></textarea>
        </div>
