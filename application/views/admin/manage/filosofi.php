	...
	<main class="max-w-7xl max-w mx-auto flex lg:py-5 pb-32 px-4 pt-4 lg:gap-8">
		...
		<div class="flex flex-col w-full">
			...
			<div class="w-full flex justify-between items-center py-3">
				<h1 class="text-xl font-bold">Filosofi logo Kabinet <span class="text-primary"><?= $landing->kabinet ?></span></h1>
				<a href="<?= site_url('admin/manage/new_filosofi/' . $landing->id) ?>" class="bg-primary p-2 text-center rounded-lg text-white flex items-center justify-center" role="button"><span class="font-bold border-2 border-white rounded-md px-1 mr-1">+</span> Data</a>
			</div>
			<div class="relative overflow-x-auto pb-6">
				<table class="table rounded-xl w-full">
					<thead>
						<tr class="border-b-2 border-slate-300 mb-2">
							<th class="text-left py-3 px-4">Makna</th>
							<th style="width: 18%;" class="text-center py-3 px-4">gambar</th>
					...
					<tbody>
						<?php foreach ($data_filosofi as $filosofi) : ?>
							<tr class="border-b hover:bg-white">
								<td class="px-4">
									<h4 class="font-medium"><?= $filosofi->nama_element ?></h4>
									<p class="text-gray-500 text-sm"><?= $filosofi->makna_element ?></p>
								</td>
								<td>
									<div class="flex justify-center p-2 items-center">
										<?php
										$logo = base_url('public/img/logo/filosofi/' . $filosofi->img_logo);
										?>
										<img src="<?= $logo ?>" alt="<?= htmlentities($filosofi->nama_element, TRUE) ?>" class="rounded-lg h-16 mx-auto">
									</div>
								</td>
								<td>
									<div class="flex flex-wrap gap-1 justify-center p-2">
										<a href="<?= site_url('admin/manage/edit_filosofi/' . $landing->id . '/'  . $filosofi->id) ?>" class="bg-green-500 rounded-lg py-1.5 px-2.5 text-white" role="button">Edit</a>
										<a href="#" data-delete-url="<?= site_url('admin/manage/delete_filosofi/' . $landing->id . '/'  . $filosofi->id) ?>" class="rounded-lg bg-rose-500 p-1 flex items-center justify-center" role="button" onclick="deleteConfirm(this)">
