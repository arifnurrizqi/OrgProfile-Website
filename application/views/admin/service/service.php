	...
	<main class="max-w-7xl max-w mx-auto flex lg:py-5 pb-32 px-4 pt-4 lg:gap-8">
		...
		<div class="flex flex-col w-full">
			...
			<div class="w-full flex justify-between items-center py-3">
				<h1 class="text-xl font-bold">Layanan organisasi, Kabinet <span class="text-primary"><?= $landing[0]->kabinet ?></span></h1>
				<a href="<?= site_url('admin/service/new/') ?>" class="bg-primary p-2 text-center rounded-lg text-white flex items-center justify-center" role="button"><span class="font-bold border-2 border-white rounded-md px-1 mr-1">+</span> Layanan</a>
			</div>
				...
				<table class="table rounded-xl w-full">
					<thead>
						<tr class="border-b-2 border-slate-300 mb-2">
							...
							<th style="width: 15%;" class="text-center py-3 px-4">Link</th>
					...
					<tbody>
						<?php foreach ($data_service as $service) : ?>
							<tr class="border-b hover:bg-white">
								<td class="px-4 py-2">
									<h4 class="font-medium"><?= $service->name_service ?></h4>
									<p class="text-gray-500 text-sm">
										<?php
										// Mengambil teks keterangan dari $service
										$fullText = $service->keterangan;

										// Menampilkan sebagian kata awal (misalnya, 100 karakter pertama)
										$partialText = substr($fullText, 0, 120);
								...
								<td class="px-4 py-2">
									<h4 class="font-medium text-center hover:text-primary"><a class="underline" href="<?= $service->link ?>" target="_blank"><u>Preview</u></a></h4>
								</td>
								<td>
									<div class="flex flex-wrap gap-1 justify-center p-2">
										<a href="<?= site_url('admin/service/edit/' . $service->id) ?>" class="bg-green-500 rounded-lg py-1.5 px-2.5 text-white" role="button">Edit</a>
										<a href="#" data-delete-url="<?= site_url('admin/service/delete/'  . $service->id) ?>" class="rounded-lg bg-rose-500 p-1 flex items-center justify-center" role="button" onclick="deleteConfirm(this)">
