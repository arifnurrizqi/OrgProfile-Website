...
<body>
  <main class="max-w-7xl max-w mx-auto flex lg:py-5 pb-32 px-4 pt-4 lg:gap-8">
    ...
    <div class="flex flex-col w-full right-0">
      ...
      <h1 class="text-xl font-bold lg:mb-2.5">Tambah Menteri</h1>

      <form action="<?= base_url() ?>admin/pengurus/submit_addMenteri" method="POST" enctype="multipart/form-data">
        <div>
        ...
        <div>
          ...
          <input type="text" name="prodi" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan program studi" value="" required />
        </div>
        <div>
          <label for="angkatan" class="w-full font-medium text-gray-900 dark:text-white">Angkatan</label>
          <input type="text" name="angkatan" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan tahun angkatan" value="" required />
        </div>
        <div>
          <label for="jabatan_ref" class="w-full font-medium text-gray-900 dark:text-white">Dibawah Kemenkoan</label>
          <select name="jabatan_ref" id="" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2 my-2">
            <option value="">-- Pilih Kemenkoan</option>
            <?php foreach ($coordinators as $kemenkoan) : ?>
              <option value="<?= $kemenkoan->id ?>">
                <?= $kemenkoan->jabatan ?> - <?= $kemenkoan->nama ?>
              </option>
            <?php endforeach ?>
          </select>
          <p class="italic text-xs font-normal mt-1 mb-3"><span class="font-bold text-rose-500">*</span>Jika pilihan tidak ada, tambahkan <b>Menko (menteri koordinator) nya</b> terlebih dahulu, sebelum menambahkan <b>Menteri. </b>Tapi jika tidak memakai sistem Menko dapat dikosongkan saja. Pilih yang PLT atau PJ Menko (jika ada)</p>
        </div>
        <div>
          <label for="jabatan" class="w-full font-medium text-gray-900 dark:text-white">Kementerian</label>
          <input type="text" name="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan nama kementerian" value="" required />
        </div>
        <div class="w-full">
          <label for="about" class="w-full font-medium text-gray-900 dark:text-white">Tentang Kementerian</label>
          <textarea type="text" name="about" rows="8" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan tentang Kementerian" required></textarea>
        </div>
        <div id="sosmed">
