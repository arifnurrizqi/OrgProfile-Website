...
<body>
  <main class="max-w-7xl max-w mx-auto flex lg:py-5 pb-32 px-4 pt-4 lg:gap-8">
    ...
    <div class="flex flex-col w-full right-0">
      ...
      <div class="p-4 rounded-xl border-2 mb-4">
        ...
        <form action="<?= base_url() ?>admin/pengurus/submit_editStaff" method="POST" enctype="multipart/form-data">
          <div>
          ...
          <div>
            ...
            <input type="text" name="prodi" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan program studi" value="<?= $pengurus->prodi; ?>" />
          </div>
          <div>
            ...
            <input type="text" name="angkatan" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan tahun angkatan" value="<?= $pengurus->angkatan; ?>" />
          </div>
          <div id="sosmed">
          ...
          <div class="w-full flex flex-wrap mb-3">
            <div class="w-full md:w-2/3 flex flex-wrap mb-2">
              <label for="foto" class="w-full mb-2 font-medium text-gray-900 dark:text-white">Upload foto <span class="text-sm italic">(Opsional)</span></label>
      ...
      <div class="p-4 rounded-xl border-2 mb-4">
        <h1 class="text-xl font-bold lg:mb-2.5">Edit Posisi Jabatan</h1>
        <form action="<?= base_url() ?>admin/pengurus/submit_editPosition" method="POST">
          <div>
            <label for="jabatan_ref" class="w-full font-medium text-gray-900 dark:text-white">Dibawah Kementerian</label>
            <input type="hidden" name="id_landing" value="<?= $landing->id; ?>">
            <input type="hidden" name="id_pengurus" value="<?= $pengurus->id; ?>">
            <select name="jabatan_ref" id="jabatan_ref" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2 mt-1 mb-3" required>
              ...
              <?php foreach ($coordinators as $kementerian) : ?>
                <option class="<?php echo (stripos($kementerian->jabatan, 'PLT') !== false || stripos($kementerian->jabatan, 'PJ') !== false) ? 'hidden' : '' ?>" value="<?= $kementerian->id ?>" value="<?= $kementerian->id ?>" data-jabatan="<?= str_replace('Menteri', '', $kementerian->jabatan) ?>">
                  <?= $kementerian->jabatan ?> - <?= $kementerian->nama ?>
                </option>
              <?php endforeach ?>
            </select>
          </div>
          <div>
            <label for="jabatan" class="w-full font-medium text-gray-900 dark:text-white">Sebagai</label>
            <input type="text" name="jabatan" id="jabatan" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:border-primary focus:outline-none focus:ring-1 block w-full p-2.5 mt-1 mb-4" placeholder="Masukan staff kementerian" value="" required />
          </div>
          <div class="flex gap-3 mt-3">
            <button type="submit" name="save" class="bg-primary py-2 px-3 text-center rounded-lg text-white mt-2">Simpan</button>
          </div>
  ...
  <script>
    ...
    jabatanRefSelect.addEventListener('change', function() {
      ...
      jabatanInput.value = 'Staff' + jabatanValue;
    });
    //confirm alert
