...
<section id="home" class="min-h-screen flex items-center px-10 dark:bg-dark">
  <div class="container relative isolate">
    ...
    <h1 class="text-3xl lg:text-4xl font-extrabold  uppercase text-dark text-center dark:text-white">kemenkoan <span class="text-primary">
        <?php
        $kemenkoan = str_replace("Menko", "", $menko->jabatan);
        echo $kemenkoan;
        ?></span>
    </h1>
  ...
  <div class="container">
    <div class="flex flex-wrap">
      <div class="mb-10 w-full px-4 lg:w-3/5">
        ...
        <h2 class="mb-5 max-w-md text-3xl font-bold text-dark dark:text-white lg:text-4xl"><?= str_replace("Menko", "Kemenkoan", $menko->jabatan) ?></h2>
        <?php
        $longText = $menko->about;
  ...
  <div class="container">
    ...
    <div class="w-full">
      ...
      <div class="flex w-full gap-6 flex-wrap justify-center px-4 xl:mx-auto xl:w-10/12">
        <div class="max-w-72 py-4 lg:px-4 px-4 rounded-3xl shadow-md dark:bg-slate-800 h-full border dark:border-dark">
          <div class="overflow-hidden rounded-3xl shadow-md px-8 dark:bg-slate-800 h-72 border dark:border-dark">
            <img src="<?php echo (!empty($menko->foto)) ?  base_url('public/img/pengurus/' . $menko->foto) : base_url('public/img/pengurus/profile.png') ?>" alt="<?= $menko->jabatan ?>" class="mt-3 object-cover shadow-sm w-full object-center h-full" />
          </div>
          <p class="mt-3 text-xl font-bold text-dark text-center mb-2 dark:text-slate-100"><?= $menko->nama ?></p>
          <p class="text-base font-medium text-secondary text-center"><?= $menko->prodi . ' ' . $menko->angkatan ?> </p>
          <!-- sosmed -->
          <div class="flex items-center justify-center mt-2 gap-2">
            <!-- Instagram -->
            <a href="https://instagram.com/<?= $menko->ig; ?>" target="_blank" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-300 text-slate-300 hover:border-primary hover:bg-primary hover:text-white">
            ...
            <!-- TikTok -->
            <a href="https://tiktok.com/@<?= $menko->tiktok; ?>" target="_blank" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-300 text-slate-300 hover:border-primary hover:bg-primary hover:text-white">
              <svg role="img" width="20" class="fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <title>TikTok</title>
            ...
            <!-- Twitter -->
            <a href="https://twiter.com/<?= $menko->twiter; ?>" target="_blank" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-300 text-slate-300 hover:border-primary hover:bg-primary hover:text-white">
              <svg role="img" width="20" class="fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <title>Twitter</title>
            ...
            <!-- Facebook -->
            <a href="https://facebook.com/<?= $menko->fb; ?>" target="_blank" class="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-300 text-slate-300 hover:border-primary hover:bg-primary hover:text-white">
              <svg role="img" width="20" class="fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="max-w-full mt-10 flex items-center justify-center">
        <h4 class="w-full lg:max-w-xl text-center rounded-full mx-2 mb-8 bg-gradient-to-tr from-slate-500 to-dark py-2 text-lg font-semibold lg:font-semibold text-white transition duration-300 ease-in-out hover:opacity-80 hover:shadow-lg dark:bg-gradient-to-tr dark:from-slate-500 dark:to-slate-300 dark:text-dark">Menteri lingkar <?= $kemenkoan ?></h4>
      </div>
      <div class="flex w-full gap-6 flex-wrap justify-center px-4 xl:mx-auto">
        <?php foreach ($data_menteri as $menteri) : ?>
          <div class="max-w-72 lg:w-1/4">
            <a href="<?php echo (stripos($menteri->jabatan, 'PLT') !== false || stripos($menteri->jabatan, 'PJ') !== false) ? '#' : base_url('kementerian/' . $menteri->id . '/' . $this->uri->segment(3)); ?>">
